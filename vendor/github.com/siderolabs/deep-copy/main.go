...
var (
	...
	headerFile       = flag.String("header-file", "", "file containing header that should be used for generation")
...
func generateFile(p *packages.Package, imports map[string]string, fn [][]byte) ([]byte, error) {
	...
	if *headerFile != "" {
		hdrFile, err := os.Open(*headerFile)
		if err != nil {
			return nil, fmt.Errorf("failed to open header file '%s': %w", *headerFile, err)
		}
		io.Copy(&file, hdrFile)
		fmt.Fprintf(&file, "\n")
	}
